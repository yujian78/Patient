...
.factory('SendEmail', function($http, ServerURL) {
  var email = function(email, date, time, docID, callback) {
    $http
    .post(ServerURL.sendemail, {"email": email, "date": date, "time": time, "docID": docID})
    .success(function(data) {
      callback(data);
    })
  }
...
.factory('ReferralFollow', function($http, ServerURL) {
  var checkRequest = function(userEmail, callback){
    $http
    .post(ServerURL.referralfollow, {"checkRF": userEmail})
    .success(function(data){
      callback(data);
    });
  }
...
.factory('DisplayReferralDoc', function($http, ServerURL) {
  var referralDocRequest = function(doctorID, userID, callback){
    $http
    .post(ServerURL.referralDocRequest, {"doctorID": doctorID, "userID": userID})
    .success(function(data){
      callback(data);
    });
  }
  ...
  return {
    referralDocRequest: referralDocRequest
  }
})
